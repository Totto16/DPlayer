    ...
    */
    ...
    parseVtt(vtt_url, callback) {
        ...
        .then((response) => {
            ...
            let data = response.data.replaceAll('\r','').split("\n").filter(a=>a.length>0)
            data.forEach((text,i)=>{
                if(text.includes('-->')){
                    let mark = {};
                    mark.text = data.length>i+1 ? data[i+1] : 'Fehler';
                    let endTime = text.split("-->")[1].trim()
                    console.log(endTime)
                    let multiplier=[1,60,60,24]// second, minute, hour, day
                    let index = 0;
                    let time = endTime.split(':').reverse().reduce((sum,act)=>{
